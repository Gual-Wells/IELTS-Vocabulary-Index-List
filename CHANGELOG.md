...
- 统一 Native Dialog 外层宽度和内部 100% 卡片，修复右侧截断、错位和搜索标题偏心；
- 手动切换字母／日期模式不再读取目标模式旧位置；
- 将自动上次位置改为手动浏览锚点：短按跳转、长按保存；
- 字母栏使用首尾警戒区提前移动，并在手动横滑结束后最终同步；
- 修复序号开启后繁体释义落在序号下方；
- 无关系条目保留不可交互关系槽位，固定日期和操作按钮列；
- 跨域来源改到一级表项右下角，与繁体释义共享双行高度；
- 日历增加年度双箭头，单箭头内移并继续按月跳转；
