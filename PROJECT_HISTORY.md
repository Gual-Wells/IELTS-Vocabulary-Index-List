...
## 5.0.0-alpha.14：[当前实现 / 待 iOS 真机]

alpha.14 以实机闭环的 iOS Bridge 根因为准收口跨设备认证：WebKit 可能把 `G:...` 式粘贴值视作 URI scheme 并把首字母变为小写。PWA 只在原值收到 401 后尝试一次首字母恢复，恢复值仍由 Bridge 精确认证，成功后才写回本机。凭据页面不再输出调试或连接状态文本，空字段提示不再被密码遮罩。

Mirror 文件选择器从状态页改为紧凑的远程文件夹：本机快照立即呈现，Bridge 目录异步校准，失败时保留快照；应用启动预热完整目录。移动端删除大状态卡与无意义空白，文件名、数量、更新时间、当前状态、选择与永久删除收在目录行内。首页按钮仍只开关当前已选文件。

弹窗恢复最顶层独占触摸和滚轮，父层可见但不可滚动，通知仍位于全部弹窗之上。Mirror 数据入口补齐坏释义清理与释义必填词域校验；词汇、短语、用法等权匹配和独立低级组件词库不变。Seed revision 8 合并此前被构建器丢弃的后续释义项，并以 Seed 7 字段基线保护个人修改。Groq 查询词与英文例句增加严格按点击生成的 Google Chirp 3 HD 发音，音频只在当前结果弹窗内临时缓存，Google TTS Key 加密留在 Bridge。Bridge 为 1.4.0，Mirror 4、Schema 6、DB 5 与本机词库格式不变。

## 5.0.0-alpha.13：[历史实现]

alpha.13 收口 alpha.12 真机暴露的跨浏览器差异：凭据框避免被 iOS 密码自动填充重写，PWA 使用 Bearer 与专用头双路传递 Device Token，Bridge 仅在私有日志保留脱敏差异证据。Durable Object 的实际 run 记录成为远程文件目录权威，避免独立索引丢失导致不同浏览器看到空文件夹。

Mirror 管理收敛为类文件夹选择器，本地快照只负责首屏速度，不冒充远程文件。提醒节点移到弹窗栈之上，设置顺序和集成资源视觉重新对齐 VIX 主题。Bridge 为 1.3.0；Mirror 4、Schema 6、DB 5、Seed revision 7 和个人数据格式不变。

## 5.0.0-alpha.12：[历史实现]

alpha.12 将 Mirror 从本机材料队列改为 Bridge 远程文件库。自动任务提交的 Result 直接成为远程文件；PWA 用轻量收件箱提醒新文件，只有打开文件时才读取完整内容。审核后的 Mirror 记录写回同一文件，管理页负责多文件选择、查看和永久删除；首页按钮只开启或关闭当前已选文件。下载请求、手动导入、待审核队列、归档与恢复均退出产品正文。

性能路径同时收紧：管理弹窗不等待 Bridge 才显示，周期轮询不再搬运完整 Result，相同修订的 Context 不重复上传；Mirror 选择只更新运行时元数据，关闭时复用完整结构投影与关系索引，开启时只构建允许条目的稀疏关系视图。嵌套父弹窗取消 alpha.11 后续实验中的缩放与虚化，恢复原尺寸、原清晰度，仅保留不可交互。Bridge 为 1.2.0；Mirror 4、Schema 6、DB 5、Seed revision 7 和个人词库格式不变。

## 5.0.0-alpha.11：[历史实现]

alpha.11 将 Mirror 从一次性候选导入扩展为按材料管理的学习切面。冻结 Context 明确声明词汇、短语、用法三类同权，Result 分成“VIX 已有匹配”和“VIX 库外候选”两层，并要求每项携带类别、重要度与材料证据。审核采用材料、结果层、分类组、单条四级复选联动；提交后只把实际勾选的已有条目与导入候选纳入当前 Mirror，不再把所有低级关联词扩散进材料视图。材料记录可切换、关闭、归档、恢复和追溯；Bridge 确认结果也不再物理删除，可从历史恢复。

原低级词关联表由 A1/A2 成员资格派生，范围过宽，会把正常基础词误当成语法噪声。本版建立独立、可审计的来源文件，人工清洗为 244 项闭类词、代词、限定词、助动词、连词、介词粒子、数量词和语法性话语成分。该表仅用于抑制单词组件关系；短语和用法永不因内部含低级词而被过滤。VIX Function 同时改为 UTF-8 字节提交，修复 PowerShell 5.1 的中文问号损坏。Bridge 为 1.1.0，Mirror Context/Result 为 v4；Schema 6、DB 5、Seed revision 7 和个人词库数据格式不变。

## 5.0.0-alpha.10.5：[当前实现 / 待真机]

alpha.10.5 修复 Mirror Context 的 Durable Object 持久化边界错误。完整 23,917 词 Context 的 JSON 为 6,579,469 字节，旧 Bridge 却把它作为单个 `context` 值写入 SQLite-backed Durable Object，实际运行时返回 `SQLITE_TOOBIG`，再被通用异常映射成“Bridge 无法完成请求”。本版将 corpus 拆成不超过 512 KiB 的不可变分片，所有分片写入成功后才原子切换活动 manifest；Agent 创建 run 时透明重组原 Mirror 3 Context。旧单值 Context 仍可读，下一次同步自动替换。Bridge 为 1.0.5；外部协议、Schema 6、DB 5、Seed revision 7 与本地个人数据不变。

## 5.0.0-alpha.10.4：[当前实现 / 待真机]

alpha.10.4 修复 alpha.10.3 的 Groq 传输兼容缺陷。Bridge 在发往 Groq 的 `/models` 与 `/chat/completions` 子请求中使用了 Cloudflare `workerd` 明确不支持的 `redirect: "error"`；该参数在线上构造请求时抛出 `TypeError`，又被统一映射成“Bridge 无法连接 Groq”，而 Node 测试接受该参数，造成错误绿灯。本版移除两处不兼容参数，并使 Bridge“测试”在输入候选 Key 时调用不持久化验证端点；Key 留空时仍验证已保存配置。已用本地 `workerd` 和故意无效的假 Key 得到 Groq HTTP 401，确认请求已真正到达上游。Bridge 为 1.0.4；Schema 6、DB 5、Seed revision 7 与 Mirror 3 不变。

## 5.0.0-alpha.10.3：[当前实现 / 待真机]

alpha.10.3 处理 alpha.10.1/10.2 Bridge 初始化中暴露的配置一致性缺陷。根因是 Worker Secret 随 Cloudflare Worker 版本生效，而加密后的 Groq Key 和 Mirror Context 存在不随版本回滚的 Durable Object；旧状态接口只检查记录存在，前端又分阶段写入本机配置，因而可能形成“Token 可用、Groq 已配置、实际无法解密或模型目录未更新”的半配置状态。本版为密钥信封加入非秘密指纹，状态接口验证解密，PWA 在 Token、Groq 模型和 Mirror Context 全链路成功后才提交本机配置。查询弹窗同步从历史四列收缩为实际两列。Schema 6、DB 5、Seed revision 7、Mirror 3 数据协议不变。完整事故复盘见 `BRIDGE_CONFIGURATION_INCIDENT_2026-09-06.md`。

## 5.0.0-alpha.10.2：[当前实现 / 待真机]

alpha.10.2 修复 alpha.10.1 真机验收暴露的“健康检查与能力检查混淆”：Bridge 测试现在在 Token/status 成功后继续验证已存 Groq Key 的解密与 `/models` 上游链路。Bridge 对密钥解密失败、上游不可达和上游 HTTP 拒绝分别给出可行动错误。UI 将嵌套工具视为替换父工具的唯一可见层，保留父状态但不露出父底栏；同时重置新层滚动、统一下载入口颜色、清理 Mirror 原生文件框和触摸查询菜单的伪焦点框。Schema、DB、Seed 与 Mirror 数据协议均不变。

## 5.0.0-alpha.10.1：[当前实现 / 待真机]

alpha.10.1 是 Bridge 接入修复补丁。它接受 12 字符以上的随机 Master Key，兼容密码管理器默认生成的十几位密码；Bridge 测试结果同时在弹窗状态区与顶部提示可见。Bridge 保存只负责验证配置、保存 Device 配置及可选的 Groq Key，不再隐式触发大体积 Mirror Context 上传和收件箱拉取；Mirror 同步继续由 Mirror 页面显式控制。Worker 部署配置声明三个必需 Secret，固定 `workers.dev` 并关闭随机 Preview URL。产品 Schema、DB、Seed 与个人数据格式均不变。

## 5.0.0-alpha.10：[当前实现 / 待真机]

alpha.10 将产品恢复为 GitHub Pages 本地优先 PWA，并把个人网络能力收敛为版本无关的薄 Bridge。Bridge 只负责 Mirror 运行队列、Groq Key 加密保存与 Groq 代理；外部任务由本地 VIX Function 通过精确 `VIX:` 协议获取冻结上下文和提交结果。Mirror 3 在 PWA 内按材料、字母、单词三级审阅，取消不写入，提交以单事务完成新词合并、词表分配、关系重建与 Mirror 安装。

Collins 生命周期在 alpha.9 结束：官方浏览器演示可用，但独立 Windows 与边缘运行时的合法 API 请求持续受到上游 Cloudflare HTML 403 challenge，无法形成稳定、可交付的服务端链路。产品运行时、查询 UI、设置 UI、发布目录与当前部署文档已全部移除该依赖；旧实现、耦合代码、测试、部署证据和结论保存在仓库外层 `history/collins-lifecycle-alpha9/` 及其独立 ZIP 中，仅供审计，不再参与构建。

ChatGPT 查询入口同批退役。官方当前提供 iOS App Intents/快捷指令，但没有可依赖的网页 URL 契约同时保证打开 ChatGPT iOS 应用并预填任意文本；为避免保留失效入口，产品只保留 Oxford 与 Groq。
...
## 4.7.1：[历史回归 / presentation成果由4.7.2继承]
...
4.5 对导航做重新建模而不是给 4.4 打补丁：

- `destructive-v3` 明确三层 ownership：VIX Automaton 负责 PUSH/POP/HOME/VIEW/JUMP；Browser Rail Adapter 负责 history slot/key/traversal；Renderer 只渲染 committed controller state。
- Collection 是唯一 recursive frame。word/phrase、alphabet/date、calendar、expanded、scroll、Entry target 都是 frame presentation state；同 Collection Search/Relation/PIN/Annotation 不 PUSH。
- VIX `token` 与 UA `NavigationHistoryEntry.key` 双身份：token 证明逻辑 frame，browserKey 证明 Safari rail slot。depth/URL/history.state 都不得替代 identity。
- 4.4 把 `history.pushState()` 写入的 classic state 再用 `NavigationDestination.getState()` 读取，属于两套状态面的错误混用；4.5 destination classifier 只读 `destination.key`。
- 真实跨 Collection PUSH 必须同步发生在用户导航命令中。Home→Collection mode reset 先同步 hydrate runtime memory，PUSH/render 后再持久化；Search 不再等 Modal 140ms exit。
- App Back 精确 `traverseTo(parent.browserKey)`；Home 精确 `traverseTo(rootBrowserKey)`。只有 committed traversal 才 destructive POP/CLEAR，因此 half-swipe cancel 不应修改栈。
- Home 不再创建 ROOT2。回原 root 后旧 A/B/C 成为 dead Forward；下一次 fresh PUSH D 由浏览器正常 session-history 规则物理截断旧 branch。
- 支持 Navigation API 时只有 `navigate` 一个 traversal owner；`popstate` 只在 fallback 平台注册，删除 4.4 的 token+timeout 去重器。
- runtime 中 `replaceState()` 只允许 boot root 初始化一次；捕获 root key 后不 rewrite live rail slot。
- 删除 70ms orphaned page transition；Renderer 不再根据 root URL/hash destructive 修改导航栈；review renderer 也无权自动 PUSH。
- Navigation runtime 不跨 PWA process/session 恢复；重新启动按产品事实从 Home 建新 root。

4.5 自动化覆盖 key classifier、parent traversal、dead Forward、Home clear、PUSH-before-await、single traversal owner 与 replaceState count；iPhone interactive visual surface / after-transition scroll / noncancelable Forward 仍由 `tests/IPHONE_REDUCED_TESTS_4.5.0.md` 真机验收。
...
7. Collection 是唯一 recursive frame；fresh cross-Collection PUSH 不读取旧页面缓存；recursive Back 按 UA browserKey 精确回父 frame并恢复离开状态。
