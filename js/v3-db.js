...
export function mergeBuiltInDomainBackup(_baseBackup, seedBackup) {
  ...
  return canonicalizeBackup({ ...seedBackup, appVersion: '4.3.0', schemaVersion: SCHEMA_VERSION });
}
...
export async function replaceWithBackup(input, { migrationNoticePending = false, expectedRevision = null } = {}) {
  if (Number(input?.schemaVersion) !== SCHEMA_VERSION) throw new Error('完整备份版本不兼容；4.3.0 仅接受 Schema 6 完整备份');
