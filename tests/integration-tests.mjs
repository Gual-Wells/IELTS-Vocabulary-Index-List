...
function buildState(backup) {
  ...
  collectionById.set(SYSTEM_GLOBAL_CONTENT_ID, { id: SYSTEM_GLOBAL_CONTENT_ID, name: '全局非结构总表', type: 'system-global-content', virtual: true });
...
const context = createEntryContext(state, dataEntry, visibleCollection.id, { appVersion: '4.5.0' });
...
for (const entry of seed.entries) {
  ...
  const ctx = createEntryContext(state, entry, collection.id, { appVersion: '4.5.0' });
...
if (globalAccess) {
  const globalContext = createEntryContext(state, globalAccess, SYSTEM_GLOBAL_WORDS_ID, { appVersion: '4.5.0' });
