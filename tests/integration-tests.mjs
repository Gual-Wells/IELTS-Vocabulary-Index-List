...
const context = createEntryContext(state, dataEntry, visibleCollection.id, { appVersion: '4.0.0' });
...
for (const entry of seed.entries) {
  ...
  const ctx = createEntryContext(state, entry, collection.id, { appVersion: '4.0.0' });
...
if (globalAccess) {
  const globalContext = createEntryContext(state, globalAccess, SYSTEM_GLOBAL_WORDS_ID, { appVersion: '4.0.0' });
