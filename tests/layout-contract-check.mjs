...
const styles = [
  ...
  'css/v4.7.0.css', 'css/v4.7.1.css', 'css/v4.7.2.css', 'css/v4.7.3.css', 'css/provider-runtime.css', 'css/v5.0.0.css',
...
const modalScrollSource = uiSource.slice(
  uiSource.indexOf('function modalScrollableTarget'),
  uiSource.indexOf('\nfunction focusableModalNodes', uiSource.indexOf('function modalScrollableTarget')),
);
assert.ok(modalScrollSource.includes('function handleModalWheel'), 'Unable to extract the modal scroll guard');
...
<div id="query-fixture" class="query-menu"><button class="query-menu-option oxford-option"><span class="ui-icon"></span><span class="query-provider-label">Oxford</span></button><button class="query-menu-option groq-option"><span class="ui-icon"></span><span class="query-provider-label">Groq</span></button></div>
<section id="provider-fixture" class="provider-result-card" style="max-width:360px"><div class="provider-headword-row"><h3 class="provider-headword">earnest</h3><button class="provider-speech-button" aria-label="播放 earnest 发音"><svg></svg></button></div><section class="provider-section"><h4 class="provider-section-label">例句</h4><div class="provider-example"><div class="provider-example-english-row"><p class="provider-example-english">She made an earnest effort to understand the problem.</p><button class="provider-speech-button" aria-label="播放例句发音"><svg></svg></button></div><p class="provider-example-translation">她认真努力地理解这个问题。</p></div></section></section>
...
<section id="file-layer" class="modal-layer" data-depth="4" data-variant="file-picker"><div class="modal-layer-backdrop"></div><form id="file-card" class="modal-card modal-card-file-picker"><header class="dialog-header"><div><h2>选择 Mirror 文件</h2></div><button class="icon-button"></button></header><div class="dialog-body"><section class="mirror-management-section mirror-explorer"><div class="mirror-explorer-toolbar"><div class="mirror-explorer-path"><span>Personal Mirror</span><span>›</span><strong>MirrorFS</strong></div><input class="mirror-explorer-search" placeholder="搜索 Mirror 文件"></div><div class="mirror-explorer-heading"><span>名称</span><span>状态与操作</span></div><div class="mirror-library"><article class="mirror-library-card"><button class="mirror-library-open"><span class="mirror-file-icon"></span><span class="mirror-file-copy"><strong>Paper A</strong><span>12 已有项 · 3 个候选</span><span>2026-09-08 22:00</span></span></button><div class="mirror-library-actions"><span class="mirror-library-state">新文件</span><button class="secondary-button compact-button">选择</button><button class="danger-button compact-button">删除</button></div></article></div></section></div></form></section></div>
...
try {
  ...
  for (const selector of ['#search-card', '#confirm-card', '#file-card']) {
    const card = await box(selector);
    assert.ok(card && card.height < 850 && Math.abs(card.x + card.width / 2 - 201) < 1.5, `${selector} geometry: ${JSON.stringify(card)}`);
  }
  for (const selector of ['#management-layer', '#search-layer', '#confirm-layer', '#file-layer']) {
  ...
  const fileCard = await box('#file-card');
  const fileRow = await box('#file-card .mirror-library-card');
  const fileToolbar = await box('#file-card .mirror-explorer-toolbar');
  assert.ok(fileCard && fileCard.height < 360, `mobile file picker should be content-sized, got ${fileCard?.height}`);
  assert.ok(fileRow && fileRow.height < 90, `mobile file row should stay compact, got ${fileRow?.height}`);
  assert.ok(fileToolbar && fileToolbar.height < 60, `mobile file toolbar should stay on one row, got ${fileToolbar?.height}`);
  ...
  const queryMenu = await box('#query-fixture');
  const queryOptions = await page.locator('#query-fixture .query-menu-option').all();
  assert.ok(queryMenu && queryMenu.width < 110, `two-provider query menu should be compact, got ${queryMenu?.width}`);
  assert.equal(queryOptions.length, 2);
  const optionWidths = await Promise.all(queryOptions.map((option) => option.evaluate((element) => element.getBoundingClientRect().width)));
  assert.ok(optionWidths.every((width) => Math.abs(width - 46) < 0.5));

  const speechButtons = await page.locator('#provider-fixture .provider-speech-button').all();
  assert.equal(speechButtons.length, 2);
  for (const control of speechButtons) {
    const speechBox = await control.boundingBox();
    assert.ok(speechBox && speechBox.width >= 44 && speechBox.height >= 44, `speech control must keep an iOS-sized target: ${JSON.stringify(speechBox)}`);
  }
  const providerFixture = await box('#provider-fixture');
  const exampleRow = await box('#provider-fixture .provider-example-english-row');
  assert.ok(providerFixture && exampleRow && exampleRow.x >= providerFixture.x && exampleRow.x + exampleRow.width <= providerFixture.x + providerFixture.width + 0.5);

  const parentLayer = page.locator('#management-layer');
  await parentLayer.evaluate((element) => { element.inert = true; });
  assert.equal(await parentLayer.evaluate((element) => getComputedStyle(element).visibility), 'visible');
  assert.equal(await parentLayer.evaluate((element) => getComputedStyle(element).pointerEvents), 'none');
  assert.equal(await page.locator('#dialog-form').evaluate((element) => getComputedStyle(element).opacity), '1');
  assert.match(await page.locator('#dialog-form').evaluate((element) => getComputedStyle(element).transform), /^(none|matrix\(1, 0, 0, 1, 0, 0\))$/);
  assert.equal(await page.locator('#search-layer').evaluate((element) => getComputedStyle(element).visibility), 'visible');
  await parentLayer.evaluate((element) => { element.inert = false; });

  const modalScrollResult = await page.evaluate((source) => {
    globalThis.eval(`
      const dialogStack = [];
      let openModalCount = 1;
      let modalTouchY = 0;
      let modalTouchFrame = null;
      ${source}
      globalThis.__modalScrollTest = { dialogStack, handleModalWheel };
    `);
    const parent = document.querySelector('#management-layer');
    const child = document.querySelector('#file-layer');
    const parentBody = parent.querySelector('.dialog-body');
    const childBody = child.querySelector('.dialog-body');
    let testScrollTop = 80;
    Object.defineProperties(childBody, {
      clientHeight: { configurable: true, value: 100 },
      scrollHeight: { configurable: true, value: 300 },
      scrollTop: { configurable: true, get: () => testScrollTop, set: (value) => { testScrollTop = value; } },
    });
    globalThis.__modalScrollTest.dialogStack.push(
      { layer: parent, body: parentBody },
      { layer: child, body: childBody },
    );
    const run = (target, deltaY) => {
      let prevented = false;
      globalThis.__modalScrollTest.handleModalWheel({ target, deltaY, preventDefault() { prevented = true; } });
      return prevented;
    };
    const parentPrevented = run(parentBody, 20);
    const childMiddlePrevented = run(childBody, 20);
    childBody.scrollTop = 200;
    const childBoundaryPrevented = run(childBody, 20);
    delete globalThis.__modalScrollTest;
    return { parentPrevented, childMiddlePrevented, childBoundaryPrevented };
  }, modalScrollSource);
  assert.deepEqual(modalScrollResult, {
    parentPrevented: true,
    childMiddlePrevented: false,
    childBoundaryPrevented: true,
  });
...
console.log('layout-contract-check: OK (402x874, 5.1.0 styles)');
