...
assert.equal(BUILTIN_SEED_REVISION, 4);
...
assert.equal(seed.appVersion, '4.0.0');
assert.equal(seed.settings.builtInSeedRevision, 4);
...
assert.equal(seed.entries.filter((item) => item.kind === 'content').length, 50);
