...
assert.equal(BUILTIN_SEED_REVISION, 8);
...
assert.equal(seed.settings.builtInSeedRevision, 8);
assert.equal(Object.hasOwn(seed.settings, 'contentSources'), false);
assert.ok(seed.entries.every((item) => Object.hasOwn(item, 'gloss') && !Object.hasOwn(item, 'glossHans') && !Object.hasOwn(item, 'glossHant') && !Object.hasOwn(item, 'glossSource')));
assert.ok(seed.memberships.every((item) => !Object.hasOwn(item, 'sourceLabel') && !Object.hasOwn(item, 'sourceOrder')));
