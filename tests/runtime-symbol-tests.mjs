...
const modalScrollStart = ui.indexOf('function modalScrollableTarget');
const modalScrollEnd = ui.indexOf('\nfunction focusableModalNodes', modalScrollStart);
const modalScrollSource = ui.slice(modalScrollStart, modalScrollEnd);
assert.ok(modalScrollSource.includes('dialogStack.at(-1)'));
assert.ok(modalScrollSource.includes('node !== frame.body'));
assert.ok(modalScrollSource.includes('modalTouchFrame !== frame'));
assert.ok(modalScrollSource.includes('function handleModalWheel'));
assert.ok(ui.includes("document.addEventListener('wheel', handleModalWheel, { passive: false, capture: true })"));
