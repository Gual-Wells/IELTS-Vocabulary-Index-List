...
assert.equal(pkg.version, '5.0.0-alpha.9');
assert.ok(index.includes('css/provider-runtime.css'));
assert.ok(index.includes('Vocabulary Index 5.0.0-alpha.9'));
...
assert.ok(sw.includes('v5.0.0-alpha.9-shell'));
...
for (const releaseDoc of ['README.md', 'DEPLOY.md', 'LOCAL_ARCHITECTURE.md', 'MIGRATION_5.0.0-alpha.9.md', 'RELEASE_5.0.0-alpha.9.md', 'TEST_REPORT_5.0.0-alpha.9.md']) {
  assert.ok(exists(releaseDoc), `alpha.9 release document missing: ${releaseDoc}`);
}
...
assert.ok(wrangler.includes('"command": "node tools/build-dist.mjs"'));
assert.ok(wrangler.includes('"keep_vars": true'));
assert.ok(wrangler.includes('"redact_query_string": true'));
assert.ok(wrangler.includes('"persist": true'));
