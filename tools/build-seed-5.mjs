...
if (Number(domainExpansions.at(-1)?.seedRevision) !== 7) throw new Error('Latest curated domain expansion revision is not the expected Seed 7 source');
...
function cleanGlossParts(value) {
  const values = Array.isArray(value) ? value : [value];
  return values.flatMap((item) => normalizeDisplayText(item).split(/[;；、]+/))
    .map((item) => normalizeDisplayText(item)).filter(Boolean);
}

function mergeGloss(candidate, value, source) {
  const incoming = cleanGlossParts(value);
  if (!incoming.length) return;
  for (const item of incoming) {
    if (!candidate.glossParts.includes(item)) candidate.glossParts.push(item);
  }
  if (source) candidate.glossSources.add(source);
  const merged = candidate.glossParts.join('；');
  if (merged.length > 240) throw new Error(`Merged gloss exceeds 240 characters: ${candidate.text}`);
  candidate.glossHans = merged;
  candidate.glossHant = toTraditional(merged);
  candidate.glossSource = [...candidate.glossSources].join(' + ');
}
...
const seed = canonicalizeBackup({
  ...
  settings: {
    ...
    migrationSource: 'seed8-complete-gloss-generation',
...
const report = {
  ...
  counts: {
    ...
    generalGlossItems: generalEntries.reduce((sum, entry) => sum + (entry.glossHant ? entry.glossHant.split(/[;；]/).length : 0), 0),
